...
export function WindowToolbar() {
  ...
  const setScreen = useQuietStore((state) => state.setScreen);
  const reset = useQuietStore((state) => state.reset);
  ...
  return (
    <header className="flex shrink-0 items-center gap-3 border-b border-black/5 bg-titlebar px-3 py-2">
      ...
      <div className="flex items-center gap-1">
        <Button
          variant={screen === "log" ? "default" : "outline"}
          size="sm"
        ...
        <Button
          variant={screen === "pack" ? "default" : "outline"}
          size="sm"
          className="h-7 rounded-md px-2 text-[12px]"
          onClick={() => setScreen("pack")}
          data-testid="nav-pack"
        >
          Evidence
        </Button>
        <Button
